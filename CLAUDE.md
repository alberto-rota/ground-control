...
**Testing**: no suite in-repo. Headless testing works via Textual's `app.run_test()` pilot, and `XDG_CONFIG_HOME` redirects config/themes/logs away from the real `~/.config/ground-control` — always set it, since `load_colors()` writes the config as a side effect of reading it. Note `MemoryWidget` intermittently fails at some geometries via a plotext legend `IndexError`; it reproduces on unmodified checkouts and is not a regression signal.
