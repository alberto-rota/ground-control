...
def _gpu_process_command_fields(cmdline, process_name="", raw_command=""):
    """Return useful command and script labels from a process argv.

    nvitop normally provides the complete command, but some launchers and
    rewritten process titles expose only an option such as ``-m``. The argv
    still contains the module or script name, so use it as the authoritative
    fallback for the labels shown in the GPU process table.
    """
    args = [str(arg) for arg in (cmdline or []) if arg is not None]
    command_from_argv = " ".join(args[:20])
    raw_command = str(raw_command or "").strip()
    if raw_command and (len(args) <= 1 or len(raw_command.split()) > 1):
        command = raw_command
    else:
        command = command_from_argv

    script = ""
    executable = (args[0] if args else process_name).lower()
    is_python = "python" in executable or "python" in str(process_name).lower()
    if is_python:
        for idx, arg in enumerate(args[1:], start=1):
            ...
            if arg.endswith(".py"):
                script = arg
                break
        if not script and len(args) > 1 and not args[1].startswith("-"):
            script = args[1][:60]
...
class SystemMetrics:
    def __init__(self, all_gpus: bool = False):
        ...
        self.devices = (
            self._get_all_gpu_devices() if (NVML_AVAILABLE and NVITOP_AVAILABLE) else []
        )
    ...
    def _init_random_memory_simulation(self):
        ...
        self.sim_swap_total = max(
            actual_swap.total, 8 * 1024**3
        )  # At least 8GB swap for demo
    ...
    def _generate_random_memory_values(self):
        ...
        swap_cycle = math.sin(
            elapsed_time * self.sim_swap_freq + self.sim_time_offset * 1.5
        )
        ...
        ram_usage_percent = max(
            0.1,
            min(
                0.9, self.sim_ram_base + self.sim_ram_amplitude * ram_cycle + ram_noise
            ),
        )
        swap_usage_percent = max(
            0.0,
            min(
                0.8,
        ...
        if platform.system() == "Linux":
            ...
            try:
                ...
                result = subprocess.run(
                    ["sensors", "-A"], capture_output=True, text=True, timeout=5
                )
            ...
            try:
                if platform.system() == "Linux":
                    with open(sensor_path, "r") as f:
                        ...
                        friendly_name = self._get_friendly_sensor_name(
                            sensor_name, sensor_path
                        )
    ...
    def _get_friendly_sensor_name(self, sensor_name: str, sensor_path: str) -> str:
                ...
                if os.path.exists(name_file):
                    ...
                    if hwmon_name == "coretemp":
                        ...
                        if os.path.exists(label_file):
                            ...
                            temp_id = os.path.basename(sensor_path).replace(
                                "_input", ""
                            )
                            if temp_id == "temp1":
                                return "CPU Package"
                        ...
                        "gpu" in hwmon_name.lower()
                        or "radeon" in hwmon_name.lower()
                        or "amdgpu" in hwmon_name.lower()
                    ):
                        return "GPU"
        ...
        if sensor_name.lower() == "composite":
            return "NVMe SSD"
        elif "core" in sensor_name.lower() and any(
            char.isdigit() for char in sensor_name
        ):
        ...
        try:
            ...
            if self.prev_cpu_stats is not None:
                ...
                t["ctx_switches_per_s"] = max(
                    0.0,
                    (stats.ctx_switches - self.prev_cpu_stats.ctx_switches) / elapsed,
                )
    ...
    def get_disk_metrics(self):
        ...
        if total_io is not None:
            ...
            total_read_speed = (
                (total_io.read_bytes - self.prev_read_bytes)
                / (1024**2)
                / disk_time_delta
            )
            total_write_speed = (
                (total_io.write_bytes - self.prev_write_bytes)
                / (1024**2)
                / disk_time_delta
            )
            ...
            if total_read_speed < 0.01 and total_io.read_bytes > self.prev_read_bytes:
                total_read_speed = (
                    0.01  # Set a minimum value if there was positive activity
                )
        ...
        for partition in partitions:
            try:
                ...
                disk_name = (
                    partition.device.split("/")[-1]
                    if "/" in partition.device
                    else partition.device.split("\\")[-1]
                )
                ...
                if disk_name in per_disk_io:
                    ...
                    if disk_name in self.prev_disk_io:
                        ...
                        read_speed = (
                            (disk_io.read_bytes - prev_data["read_bytes"])
                            / (1024**2)
                            / disk_time_delta
                        )
                        write_speed = (
                            (disk_io.write_bytes - prev_data["write_bytes"])
                            / (1024**2)
                            / disk_time_delta
                        )
                        ...
                        if (
                            read_speed < 0.01
                            and disk_io.read_bytes > prev_data["read_bytes"]
                        ):
                        ...
                        if (
                            write_speed < 0.01
                            and disk_io.write_bytes == prev_data["write_bytes"]
                        ):
                            write_speed = 0
                    ...
                    total_disk_space = all_partitions_total - partition_total.get(
                        partition.mountpoint, 0
                    )
    ...
    def get_network_metrics(self):
        ...
        download_speed = (
            (net_io_counters.bytes_recv - self.prev_net_bytes_recv)
            / (1024**2)
            / time_delta
        )
        upload_speed = (
            (net_io_counters.bytes_sent - self.prev_net_bytes_sent)
            / (1024**2)
            / time_delta
        )
    ...
    def _throttle_reasons(self, device):
        ...
        for suffix, label, is_severe in self._THROTTLE_BITS:
            bit = getattr(pynvml, f"nvmlClocksThrottleReason{suffix}", None) or getattr(
                pynvml, f"nvmlClocksEventReason{suffix}", None
            )
    ...
    def get_gpu_metrics(self):
        ...
        for device in self.devices:
            try:
                ...
                logger.warning(
                    "GPU collection failed for %r: %s",
                    getattr(device, "index", "?"),
                    err,
                )
                continue
        return gpu_metrics
            ...
            try:
                ...
                if procs is not None:
                    for proc in procs.values():
                        try:
                            if (
                                pids_on_this_device is not None
                                and proc.pid not in pids_on_this_device
                            ):
                                continue
                            name = (
                                proc.name() if proc.name() is not NA else str(proc.pid)
                            )
                            gpu_mem = (
                                proc.gpu_memory_human()
                                if proc.gpu_memory_human() is not NA
                                else "N/A"
                            )
                            try:
                            ...
                            try:
                                ...
                                if cmdline:
                                    ...
                                    command, script = _gpu_process_command_fields(
                                        cmdline,
                                        name,
                                        raw_cmd if raw_cmd is not NA else "",
                                    )
                            except Exception:
                                pass
                            cpu_pct = None
                            ...
                            try:
                                ...
                                if host is not NA and host is not None:
                                    ...
                                    cpu_pct = (
                                        f"{cp:.1f}%"
                                        if cp is not NA and cp is not None
                                        else "N/A"
                                    )
                                    try:
    ...
    def get_memory_metrics(self):
        ...
        swap_info = (
            psutil.swap_memory()
        )  # Shape: namedtuple with total, used, free, percent, sin, sout
        ...
        try:
            ...
            memory_data["commit_ratio"] = (
                committed_as / commit_limit if commit_limit > 0 else 0
            )
        except:
            pass
        ...
        try:
            ...
            mock_processes = [
                {
                    "pid": 1234,
                    "name": "chrome",
                    "memory_percent": random.uniform(15, 25),
                },
                {
                    "pid": 5678,
                    "name": "firefox",
                ...
                {"pid": 9012, "name": "code", "memory_percent": random.uniform(8, 15)},
                {
                    "pid": 3456,
                    "name": "python",
                    "memory_percent": random.uniform(5, 12),
                },
                ...
                {"pid": 6789, "name": "mysql", "memory_percent": random.uniform(1, 4)},
                {
                    "pid": 1357,
                    "name": "nginx",
                    "memory_percent": random.uniform(0.5, 2),
                },
            ]
            ...
            for proc in mock_processes:
                proc["memory_rss"] = int(
                    memory_info.total * proc["memory_percent"] / 100
                )
                proc["memory_vms"] = int(
                    proc["memory_rss"] * 1.5
                )  # VMS typically larger than RSS

            # Sort by memory percent and take top 10
            memory_data["top_processes"] = sorted(
                mock_processes, key=lambda x: x["memory_percent"], reverse=True
            )[:10]
        except:
    ...
    def _get_visible_gpu_filter(self):
        ...
        for var in (
            ...
            if var in os.environ:
                ...
                logger.info(
                    "GPU visibility from %s=%r -> %r", var, os.environ.get(var), parsed
                )
                return parsed
        return None
    ...
    def _device_accessible(device) -> bool:
        ...
        try:
            ...
            logger.info(
                "Skipping inaccessible GPU %r: %s", getattr(device, "index", "?"), err
            )
            return False
    ...
    def _get_all_gpu_devices(self) -> list:
        ...
        if gpu_filter == "none":
            logger.info(
                "No GPUs visible to this session (CUDA_VISIBLE_DEVICES/Slurm allocation is empty)"
            )
            return []
        ...
        logger.info(
            "Detected %d usable GPU device(s)%s",
    ...
    def _update_memory_history(self, memory_info):
        ...
        self.memory_history["used"].append(
            memory_info.used / (1024**3)
        )  # Convert to GB
        self.memory_history["free"].append(
        ...
        buffers = (
            memory_info.buffers / (1024**3) if hasattr(memory_info, "buffers") else 0
        )
        ...
        if len(self.memory_history["timestamps"]) > self.max_history_points:
            for key in self.memory_history:
                ...
                if key != "total":
                    self.memory_history[key] = self.memory_history[key][
