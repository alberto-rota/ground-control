...
class TemperatureWidget(MetricWidget):
        ...
        try:
            ...
            for sensor_name, temp in temperatures.items():
                if sensor_name not in self.temperature_histories:
                    self.temperature_histories[sensor_name] = deque(maxlen=self.history_size)
            ...
            sorted_sensors = sorted(
                ...
                key=lambda x: (
                    min(sensor_priorities.get(key, 10) for key in sensor_priorities.keys() 
                        if key in x[0].lower()),
                    -max(x[1]) if x[1] else 0
                )
            )
            
            colors = ["red", "orange3", "yellow", "green", "cyan", "blue"]
            
            for i, (sensor_name, history) in enumerate(sorted_sensors[:4]):
                if history:
                    color = colors[i % len(colors)]
                    # Create short label for legend
                    short_name = sensor_name.replace("_", "").replace(" ", "")[:8]
                    plt.plot(list(history), marker="braille", label=short_name, color=color)
            ...
            if num_ticks > 1:
                ...
                y_labels = [f"{temp:.0f}°C" for temp in y_ticks]
            ...
            if max_temp > 80:
                plt.hline(80, color="red", linestyle="--")  # Warning line
            if max_temp > 60:
                plt.hline(60, color="orange3", linestyle="--")  # Caution line
            
            result = ansi2rich(plt.build()).replace("\x1b[0m", "").replace("──────┐", "──°C──┐")
            return result
    ...
    def update_content(self, temperatures: dict):
        ...
        try:
            ...
            temp_plot.update(self.get_temperature_plot(temperatures))
            
            # Update the temperature bars
            temp_bars = self.query_one("#temp-bars")
            bar_width = getattr(self, "plot_width", 40)
            temp_bars.update(self.create_temperature_bars(temperatures, bar_width))
